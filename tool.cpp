...
//            qInfo()<<a_block.text;
    ...
    std::string translatorStr='\"'  +mapperFileInfo.absoluteFilePath().toStdString()+ '\"'  
    ...
    cmd.start(translatorStr.c_str());
    cmd.waitForFinished();
    qInfo()<<"Save Pressed";
    bool fileExists = QFileInfo::exists(filepaths+"/HindiTranslated.xml") && QFileInfo(filepaths+"/HindiTranslated.xml").isFile();
    while(!fileExists){
        fileExists = QFileInfo::exists(filepaths+"/HindiTranslated.xml") && QFileInfo(filepaths+"/HindiTranslated.xml").isFile();
    }
    QFile transcriptFile3(filepaths+"/HindiTranslated.xml");
